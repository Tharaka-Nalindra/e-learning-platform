...
import TaskModel from "../model/coursetaskModel.js";
...
const updateevent = async (req, res) => {
  ...
  try {
    const result = await EventModel.findByIdAndUpdate(eventId, updatedEvent, {
      new: true,
    });
    if (result) {
      res.json(result);
...
const createTask = async (req, res) => {
  ...
  try {
    const task = await TaskModel.create({ taskName });
